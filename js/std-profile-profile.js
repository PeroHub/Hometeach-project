...
class Store {
  ...
  static getToken(){
    ...
    else{tokens = JSON.parse(localStorage.getItem('tokens'))}
  }
}
...
var actionControl = (function(){
  ...
  return{
    ...
    getUserDetails: function(){
      const userFirstname = document.querySelector('.user-profile-firstname').value;
      const userLastname = document.querySelector('.user-profile-lastname').value;
      const userBio = document.querySelector('.user-profile-bio').value;
      const userLanguage = document.querySelector('.user-profile-language').value;
      return{
    ...
    postUserDetails: async function(details){
      const token = "";
      await fetch('https://dapphome.herokuapp.com/api/teachers/14')
...
// proper category box event listener
  ...
  // event for adding names and profile set up
  document.querySelector('.save-form-btn').addEventListener('click', function(e){
    
    const userDetails = acCrtl.getUserDetails();
   // const userToken = Store.getToken();
    acCrtl.postUserDetails(userDetails);
...
// api fetch  https://dapphome.herokuapp.com/api/students
// const email = "ken@gmail.com"
// async function getUserData(){
//   await fetch('https://dapphome.herokuapp.com/api/users/')
//   .then((res) =>{
//     return res.json()
//   })
//   .then((data) =>{
